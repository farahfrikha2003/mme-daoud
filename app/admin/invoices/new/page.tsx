...
export default function AdminNewInvoicePage() {
  ...
  const submitInvoice = async (e: React.FormEvent) => {
    ...
    const payload = {
      customer: {
        ...
        tvaNumber: customer.tvaNumber || undefined,
      },
      lines: lines.map((l) => ({
    ...
    try {
      ...
      if (data.success) {
        router.push(data.data?.id ? `/admin/invoices/${data.data.id}` : '/admin/invoices');
      } else {
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.invoiceFormWrapper}>
        <div style={{ background: 'var(--color-bg-white)', borderRadius: 'var(--radius-lg)', boxShadow: '0 4px 20px rgba(201, 169, 141, 0.08)', border: '1px solid rgba(201, 169, 141, 0.15)', overflow: 'hidden' }}>
          <div className={styles.invoiceFormHeader}>
            <h2 className={styles.modalTitle}>Nouvelle facture</h2>
          </div>
          <form onSubmit={submitInvoice}>
            <div className={styles.invoiceFormBody}>
              ...
              <section className={styles.invoiceFormSection}>
                ...
                <div className={styles.invoiceFormGrid}>
                  ...
                  <div className={`${styles.formGroup} ${styles.formGroupFull}`}>
                    ...
                    <input type="date" required value={dueDate} onChange={(e) => setDueDate(e.target.value)} className={styles.searchInput} />
                  </div>
                </div>
              </section>
              <hr className={styles.invoiceFormDivider} />
              <section className={styles.invoiceFormSection}>
            ...
            <div className={styles.invoiceFormActions}>
              <Link href="/admin/invoices" className={styles.viewButton}>Annuler</Link>
              <button type="submit" disabled={saving} className={styles.addButton}>{saving ? 'Création...' : 'Créer la facture'}</button>
            </div>
          </form>
        </div>
      </div>
